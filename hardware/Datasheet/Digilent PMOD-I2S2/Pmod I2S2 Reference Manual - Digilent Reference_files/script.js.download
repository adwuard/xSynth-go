jQNew(document).ready(function () {
  ...
  dw_page.insituPopup = function (target, popup_id) {
    ...
    if (!$el.hasClass("popover-trigger")) {
      content = jQNew($el.attr("href"))
        .closest("div.fn")
        .html()
        .replace(/((^|\s*,\s*)<sup>.*?<\/sup>)+\s*/gi, "")
        .replace(/\bid=(['"])([^"']+)\1/gi, 'id="insitu__$2');
      $el
        .popover({
          content: content,
          toggle: "popover",
    ...
    if (headerName) {
      jQNew(this).prepend(
        '<a href="#' +
          ...
          headerName +
          '_anchor"' +
          ' aria-hidden="true" class="anchor"><span class="glyphicon glyphicon-link"></span></a>'
      );
    }
  });
}
